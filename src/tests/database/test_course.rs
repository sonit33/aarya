...
async fn test_create_course() {
	...
	let result = Course::create(&pool, "Test Course", Some("This is a test course")).await;
	assert!(result.is_ok());
...
async fn test_update_course() {
	...
	let result = Course::update(&pool, course_id, "Updated Course", Some("Updated description")).await;
	assert!(result.is_ok());
