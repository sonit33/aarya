...
async fn create_referenced_objects_for_test_question(pool: &MySqlPool) -> (i32, i32) {
	let course_result = Course::create(pool, "Course for TestQuestion", Some("Course description")).await.unwrap();
	...
	let question_result = Question::create(pool, course_id, "Question for TestQuestion", "Answers for TestQuestion", Some(4), Some(2), Some("Difficulty reason"), Some("Answer explanation"), Some("Hint for question"), 1).await.unwrap();
