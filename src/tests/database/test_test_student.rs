...
async fn create_referenced_objects_for_test_student(pool: &MySqlPool) -> (i32, i32) {
	// Create a course
	let course_result = Course::create(pool, "Course for TestStudent", Some("Course description for TestStudent tests")).await.unwrap();
...
async fn test_create_test_student() {
	...
	let result = TestStudent::create(&pool, test_id, student_id, Some("AI feedback for TestStudent")).await;
	assert!(result.is_ok());
...
async fn test_delete_test_student() {
	...
	let _ = TestStudent::create(&pool, test_id, student_id, Some("AI feedback for deletion TestStudent")).await.unwrap();
