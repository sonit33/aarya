...
async fn create_objects_for_tutoring(pool: &MySqlPool) -> (i32, i32, i32) {
	// Create a course
	let course_result = Course::create(pool, "Course for Tutoring", Some("This is a course for tutoring sessions.")).await.unwrap();
	let course_id = course_result.last_insert_id() as i32;

	// Create a student
	let student_result = Student::create(pool, "Student for Tutoring", "student.tutoring@example.com", "password123", 1, 1, 1).await.unwrap();
...
async fn test_delete_tutoring_session() {
	...
	let create_result = Tutoring::create(&pool, course_id, student_id, teacher_id, 2).await.unwrap();
...
async fn test_update_tutoring_session() {
	...
	let create_result = Tutoring::create(&pool, course_id, student_id, teacher_id, 1).await.unwrap();
	let tutoring_id = create_result.last_insert_id() as i32;

	// Update the tutoring session
	let update_result = Tutoring::update(&pool, tutoring_id, course_id, student_id, teacher_id, 2).await;
