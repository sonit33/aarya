...
async fn setup_db() -> (Pool<MySql>, String) {
    ...
    VerificationCode::create_or_update_student_code(&pool, student_id, "12345678").await.unwrap();
...
async fn test_verify_email_successful() {
    ...
    let mut app = test::init_service(
        App::new().app_data(web::Data::new(pool.clone())).service(verify_email_post)
    ).await;
