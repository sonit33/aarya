...
impl TestEntity {
    ...
    pub async fn create_test_chapters(pool: &MySqlPool, test_id: u32, chapter_ids: Vec<u32>) -> EntityResult<SuccessResultType> {
    ...
    pub async fn create_test_questions(pool: &MySqlPool, test_id: u32, question_ids: Vec<u32>) -> EntityResult<SuccessResultType> {
        let mut results: Vec<u64> = Vec::new();
    ...
    pub async fn read_all(pool: &MySqlPool) -> EntityResult<Vec<TestWithCourseChapter>> {
    ...
    pub async fn read_by_course(pool: &MySqlPool, course_id: u32) -> EntityResult<Vec<TestWithCourseChapter>> {
        let tests = sqlx::query_as::<_, TestWithCourseChapter>(
            r#"
    ...
    // then return the struct as a vector
    pub async fn read_by_course_chapter(pool: &MySqlPool, course_id: u32, chapter_id: u32) -> EntityResult<Vec<TestWithCourseChapter>> {
        let tests = sqlx::query_as::<_, TestWithCourseChapter>(
            r#"
            SELECT t.test_id, t.name as test_name, t.kind as test_kind, t.description as test_description, t.course_id, c.name as course_name, ch.chapter_id, ch.name as chapter_name
            FROM test t
            ...
            JOIN chapter ch ON tc.chapter_id = ch.chapter_id
            JOIN course c ON t.course_id = c.course_id
            WHERE t.course_id = ? AND ch.chapter_id = ?
        "#,
        )
        .bind(course_id)
        .bind(chapter_id)
        ...
        match tests {
            Ok(result) => EntityResult::Success(result),
            Err(e) => EntityResult::Error(DatabaseErrorType::QueryError("Failed to read test by course and chapter".to_string(), e.to_string())),
        }
    }
}
