    ...
    match File::open(file_path) {
        Ok(f) => {
            ...
            match from_reader(reader) {
                ...
                Err(e) => JsonOpsResult::Error(JsonErrorTypes::FileReadError(format!("{}", e), file_path.to_string())),
            }
        }
        Err(e) => JsonOpsResult::Error(JsonErrorTypes::FileOpenError(format!("{}", e), file_path.to_string())),
    }
}
    ...
    match JSONSchema::options().with_draft(Draft::Draft7).compile(&schema) {
        Ok(c) => match c.validate(&data) {
            ...
            Err(e) => {
                ...
                JsonOpsResult::Error(JsonErrorTypes::ValidationError("Validation failed".to_string()))
            }
        },
        Err(e) => JsonOpsResult::Error(JsonErrorTypes::SchemaCompilationError(format!("{}", e), schema_path.to_string())),
    }
}
...
pub fn validate_json_text(schema_path: &str, json_text: &str) -> JsonOpsResult<bool> {
    ...
    let data: Value = match serde_json::from_str(json_text) {
        Ok(data) => data,
        Err(e) => {
            return JsonOpsResult::Error(JsonErrorTypes::JsonParseError(format!("{}", e), json_text.to_string()));
        }
    };
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    let file = match File::open(file_path) {
        Ok(f) => f,
        Err(e) => {
            return JsonOpsResult::Error(JsonErrorTypes::FileOpenError(format!("{}", e), file_path.to_string()));
        }
    };
    ...
    let data: Vec<T> = match serde_json::from_reader(reader) {
        Ok(d) => d,
        Err(e) => {
            return JsonOpsResult::Error(JsonErrorTypes::JsonParseError(format!("{}", e), file_path.to_string()));
        }
    };
