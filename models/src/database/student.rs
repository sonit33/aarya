...
impl Student {
    pub async fn create(&self, pool: &MySqlPool) -> Result<MySqlQueryResult, Error> {
        ...
        let res = sqlx::query("INSERT INTO students (id_hash, first_name, email_address, email_hash, pass_hash, over_13, email_verified, account_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(id_hash)
    ...
    pub async fn read(&self, pool: &MySqlPool) -> Result<Option<Student>, Error> {
        let student = sqlx::query_as::<_, Student>(
            "SELECT student_id, id_hash, first_name, email_address, pass_hash, email_hash, over_13, email_verified, account_active, added_timestamp FROM students WHERE student_id = ?"
        )
    ...
    pub async fn read_password(&self, pool: &MySqlPool) -> Result<String, Error> {
        let student = sqlx::query_as::<_, Student>("SELECT pass_hash FROM students WHERE email_hash = ? or email_address = ?")
            .bind(&self.email_hash)
            .bind(&self.email_address)
            .fetch_optional(pool)
            .await;
    ...
    pub async fn update_account(&self, pool: &MySqlPool) -> Result<MySqlQueryResult, Error> {
        let res = sqlx::query("UPDATE students SET email_verified = ?, account_active = ? WHERE student_id = ?")
            .bind(&self.email_verified)
            .bind(&self.account_active)
            .bind(&self.student_id)
            .execute(pool)
    ...
    pub async fn update_email_hash(&self, pool: &MySqlPool) -> Result<MySqlQueryResult, Error> {
        ...
        let res = sqlx::query("UPDATE students SET email_hash = ? WHERE student_id = ?").bind(email_hash).execute(pool).await;
        match res {
            Ok(result) => Ok(result),
            Err(e) => Err(e)
        }
    ...
    pub async fn delete(&self, pool: &MySqlPool) -> Result<MySqlQueryResult, Error> {
        let res = sqlx::query("DELETE from students where student_id = ?").bind(&self.student_id).execute(pool).await;
        match res {
            Ok(result) => Ok(result),
            Err(e) => Err(e)
        }
